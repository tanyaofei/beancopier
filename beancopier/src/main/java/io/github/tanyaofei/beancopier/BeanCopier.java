 ...
 * <p>
 *   这个 BeanCopier 生成的转换器将会选取拷贝来源或者拷贝目标的类加载器，这取决于哪个是更低层级，如果两者的类加载器没有关联关系，则抛出 {@link ConverterGenerateException}
 * </p>
...
public final class BeanCopier {
  ...
  /**
   * 拷贝对象, 在拷贝完之后将会调用 consumer 进行回调操作
   * <pre>{@code
   ...
   *
   * @param sources  克隆对象集合, 不能为 null。如果元素为 null 则拷贝出来的元素也为 null
   * @param consumer 每克隆一次对象都会调用一次 consumer，要注意如果元素为 null 则 consumer 中的参数也为 null
   * @param <T>      克隆对象类型
   * @return 被克隆出来的对象集合
  ...
  /**
   * 批量拷贝对象，每一个对象拷贝完之后都会调用 consumer
   *
